...
Game.prototype.reset = function(){
    this.forEachPlayer(function(player){ delete player.isDead; });
};
Game.prototype.state = function(){
    var players = [];
    this.forEachPlayer(function(player){ players.push(player); });
    return {
        'players': players.filter(function(player){ return !player.isDead; }.bind(this)).map(toState),
        'dead': players.filter(function(player){ return player.isDead; }).map(toState)
    };
};
